...
class OBJLoader {
    ...
    load() {
        ...
        for (let i = 0; i < position_indices.length; i++) {
            let positionIdx = position_indices[i] * 3;
            let normalIdx = position_indices[i] * 3;
            vertex_data.push(vertex_positions[positionIdx], vertex_positions[positionIdx + 1], vertex_positions[positionIdx + 2]);
            vertex_data.push(vertex_normals[normalIdx], vertex_normals[normalIdx + 1], vertex_normals[normalIdx + 2]);
        }
    ...
    parseVec3(vec_string) {
        ...
        const tokens = vec_string.split(' ');
        if (tokens.length < 4) {
            throw 'Invalid OBJ format for vertex position or normal: ' + vec_string;
        }
        return [parseFloat(tokens[1]), parseFloat(tokens[2]), parseFloat(tokens[3])];
    }
    ...
    parseFace(face_string, entry_index)
    {
        ...
        const tokens = face_string.split(' ');

        if (tokens.length < 4) {
            throw 'Invalid OBJ format for face: ' + face_string;
        }
        ...
        for (let i = 1; i <= 3; i++) {
            ...
            if (vertex.length !== 3) {
                throw 'Invalid OBJ format for face vertex: ' + tokens[i];
            }
    
            const index = parseInt(vertex[entry_index]);
            if (isNaN(index)) {
                throw 'Invalid OBJ format for face vertex index: ' + vertex[entry_index];
            }
    
            indices.push(index - 1); // OBJ uses 1-based indices, so we subtract 1 to convert to 0-based
        }
    
        if (entry_index === 0) {
            // For vertex indices, return 3 indices for vertices
            return indices;
        } else if (entry_index === 2) {
            // For normal indices, return 3 indices for normals
            return indices;
        } else {
            // Handle texture indices if needed (entry_index === 1)
            // You can add code here if you want to parse texture indices as well
            return [];
        }
    }
